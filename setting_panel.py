...
class Api:
    ...
    def check_new_available(self, pack_name: str, key: str) -> bool:
        return (
            pack_name in self.program.abbreviations
            and key
            and key
            not in (
                key
                for pack in self.program.abbreviations.values()
    ...
    def check_change_available(self, pack_name: str, key: str, new_key: str) -> bool:
        ...
        return new_key not in (
            key for pack in self.program.abbreviations.values() for key in pack.keys()
        )
    ...
    def get_autogenerated_abbreviations(self, pack_name: str) -> list[tuple[str, str]]:
        return list(
            filter(
                lambda x: x[1][1],
                self.program.abbreviations.get(pack_name, dict()).items(),
            )
        )
...
def start_setting_panel(program):
    webview.create_window(
        "Aicé Setting Panel", "res/setting_panel/index.html", js_api=Api(program)
    )
    webview.start()
